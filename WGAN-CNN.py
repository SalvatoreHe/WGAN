...
# 创建文件夹
if not os.path.exists('./img_WGAN-CNN'):
    os.mkdir('./img_WGAN-CNN')
...
for epoch in range(num_epoch):
    for i, (img, _) in enumerate(dataloader):
        ...
        if epoch == 0:
            ...
            save_image(real_images, './img_WGAN-CNN/real_images.png')

        fake_images = to_img(fake_img.cpu().data)
        save_image(fake_images, './img_WGAN-CNN/fake_images-{}.png'.format(epoch + 1))

# 保存模型
torch.save(G.state_dict(), './generator_WGAN-CNN.pth')
torch.save(D.state_dict(), './discriminator_WGAN-CNN.pth')